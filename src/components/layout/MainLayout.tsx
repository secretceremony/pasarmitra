  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row font-sans text-foreground overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen relative bg-background">
        {/* Top Navbar */}
        <header className="sticky top-0 z-40 bg-background/80 backdrop-blur-2xl border-b border-border px-8 py-4 flex items-center gap-12 h-24">
          <button className="md:hidden p-3 text-foreground bg-card rounded-2xl border border-border" onClick={() => setMobileMenuOpen(true)}>
            <Menu size={24} />
          </button>
          ...
          <div className="flex-1 max-w-3xl flex items-center gap-4">
             <div className="relative flex-1 group">
                ...
                <input 
                  ...
                  className="w-full bg-card/60 border border-border/50 focus:border-primary/40 focus:bg-card px-14 py-4 rounded-3xl text-sm transition-all focus:outline-none shadow-sm font-bold tracking-tight"
                />
             </div>
             <Button variant="outline" className="h-14 px-8 rounded-3xl border-border bg-card/40 flex gap-3 font-black text-sm hover:border-primary/30 transition-all">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <Button 
                ...
                size="icon" 
                className={cn(
                  "h-14 w-14 rounded-[1.25rem] border-border bg-card/40 relative hover:border-primary/30 transition-all",
              ...
              >
                 <Bell size={24} className={cn("text-muted-foreground", !showNotifications && unreadCount > 0 && "animate-bounce")} />
                 {unreadCount > 0 && <span className="absolute top-4 right-4 w-2.5 h-2.5 bg-primary rounded-full border-2 border-background shadow-[0_0_10px_rgba(34,197,94,0.3)]" />}
              </Button>
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div
                    ...
                    className="absolute top-20 right-0 z-[60]"
                  >
                    <NotificationCenter />
                  </motion.div>
                )}
            ...
            <Button variant="outline" size="icon" className="h-14 w-14 rounded-[1.25rem] border-border bg-card/40 hover:border-primary/30 transition-all">
               <MessageSquareText size={24} className="text-muted-foreground" />
            </Button>
            <Button 
              variant="outline" 
              size="icon" 
              className={cn("h-14 w-14 rounded-[1.25rem] border-border bg-card/40 hover:border-primary/30 transition-all relative", rightSidebarOpen && "bg-primary/10 border-primary/20 text-primary")}
              onClick={() => {
            ...
            >
               ...
               {totalItems() > 0 && (
                 <span className="absolute -top-1 -right-1 w-6 h-6 bg-primary text-primary-foreground rounded-full border-2 border-background flex items-center justify-center text-[10px] font-black group-hover:scale-110 transition-transform">
                   {totalItems()}
                 </span>
               )}
            </Button>
            ...
            <div className="flex items-center gap-4 pl-2 group cursor-pointer">
              ...
              <div className="w-14 h-14 rounded-2xl bg-primary/20 p-0.5 border-2 border-transparent group-hover:border-primary transition-all overflow-hidden shadow-lg">
                 <div className="w-full h-full rounded-[14px] bg-primary flex items-center justify-center text-primary-foreground font-black text-xl">
