...
const MOCK_CHATS = [
  { id: 'room-1', partner: 'Warung Barokah', lastMsg: 'Can we get 5% more discount for 100 boxes?', time: '14:20', unread: 2, online: true },
  { id: 'room-2', partner: 'Toko Kelontong Jaya', lastMsg: 'Payment for INV-092 has been sent.', time: '09:12', unread: 0, online: false },
  { id: 'room-3', partner: 'Minimarket Sejahtera', lastMsg: 'When is the next shipment for Sugar?', time: 'Yesterday', unread: 0, online: true },
];

export const ChatNegotiation = () => {
  const [activeChat, setActiveChat] = useState(MOCK_CHATS[0]);
  const [inputText, setInputText] = useState('');
  const { messages, isLoading, sendMessage, isTyping, setTyping } = useChat(activeChat.id);
         ...
         <div className="flex-1 bg-card border border-border/50 rounded-[3rem] shadow-2xl flex flex-col overflow-hidden relative">
               ...
               <div className="flex items-center gap-6">
                  ...
                  <div>
                     ...
                     <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-muted-foreground">{activeChat.online ? 'Active Now' : 'Offline'}</span>
                     </div>
                  </div>
               </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar">
               <div className="flex justify-center">
                  <span className="px-4 py-1.5 bg-muted/40 rounded-full text-[10px] font-black text-muted-foreground uppercase tracking-widest">Today</span>
               </div>

               {isLoading ? (
                 <div className="flex flex-col items-center justify-center h-full gap-4 text-muted-foreground">
                    <Loader2 className="animate-spin" size={32} />
                    <p className="font-black text-xs uppercase tracking-widest">Syncing Secure Channel...</p>
                 </div>
               ) : messages.length === 0 ? (
                 <div className="flex flex-col items-center justify-center h-full text-center gap-4 py-20">
                    ...
                    <div className="space-y-1">
                       ...
                       <p className="text-sm text-muted-foreground font-medium max-w-[240px]">Start the negotiation by sending a message below.</p>
                    </div>
                 </div>
               ) : (
                 messages.map((msg) => {
               ...
               <div className="flex justify-center py-6">
                  ...
                  >
                     ...
                     <div className="flex items-center gap-4">
                        ...
                        <h4 className="font-black text-lg text-white">Special Pricing Offer</h4>
                     </div>
                     <p className="text-sm text-white/50 font-medium">Create a custom price for this order to close the partnership.</p>
                     <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-primary uppercase">Current Tier</label>
                           <p className="text-xl font-black text-white">Rp 142.000</p>
                        </div>
                         <div className="space-y-2">
                           <label className="text-[10px] font-black text-rose-400 uppercase">Target Price</label>
                     ...
                     <Button className="w-full h-12 bg-primary text-primary-foreground font-black rounded-xl shadow-xl shadow-primary/20">Send Counter Offer</Button>
                  </motion.div>
               </div>
            </div>
