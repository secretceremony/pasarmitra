...
const MOCK_REVENUE = [
  { day: 'Mon', revenue: 120000000 },
  { day: 'Tue', revenue: 154000000 },
  { day: 'Wed', revenue: 110000000 },
  { day: 'Thu', revenue: 210000000 },
  { day: 'Fri', revenue: 180000000 },
  { day: 'Sat', revenue: 250000000 },
  { day: 'Sun', revenue: 320000000 },
];

const REVENUE_BREAKDOWN = [
  { name: 'Subscription', value: 40, color: '#2C3B4D' },
  { name: 'Commission', value: 35, color: '#FFB162' },
  { name: 'Ad Revenue', value: 15, color: '#A35139' },
  { name: 'Misc', value: 10, color: '#C9C1B1' },
];
    ...
    <div className="space-y-12">
      <div className="flex items-center justify-between">
         <div className="space-y-1 border-l-4 border-emerald-500 pl-8 py-2">
            <h1 className="text-4xl font-black tracking-tighter">Financial Treasury</h1>
            <p className="text-muted-foreground font-medium">Monitoring platform liquidity, commissions, and payout cycles.</p>
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
         {[
           { label: 'Platform Revenue', value: 'Rp 4.2B', change: '+12.5%', icon: Wallet, color: 'text-emerald-500' },
           { label: 'Pending Payouts', value: 'Rp 820M', change: '84 Items', icon: Clock, color: 'text-amber-500' },
           { label: 'Escrow Holdings', value: 'Rp 12.8B', change: '+Rp 250M', icon: ShieldCheck, color: 'text-blue-500' },
           { label: 'Avg Commission', value: '4.2%', change: 'Steady', icon: TrendingUp, color: 'text-primary' }
         ].map((stat, i) => (
           <motion.div
             key={i}
         ...
         <div className="lg:col-span-3 bg-card border border-border/50 rounded-[3rem] p-10 shadow-xl space-y-8">
            <div className="flex justify-between items-center">
               ...
               <div className="flex bg-muted/30 p-1 rounded-xl">
                  {['7D', '30D', '90D'].map(t => (
                    <button key={t} className={cn("px-4 py-2 text-[10px] font-black rounded-lg uppercase tracking-widest", t === '7D' ? "bg-primary text-primary-foreground shadow-lg" : "text-muted-foreground")}>{t}</button>
                  ))}
               </div>
            </div>
            ...
            <div className="h-[400px]">
               <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={MOCK_REVENUE}>
                     ...
                     />
                     <Tooltip 
                        formatter={(v: number) => [`Rp ${v.toLocaleString()}`, 'Revenue']}
         ...
         <div className="lg:col-span-2 bg-[#1B2632] rounded-[3rem] p-10 text-[#EEE9DF] shadow-2xl flex flex-col justify-between">
            ...
            <div className="h-[300px] relative">
               ...
               <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">Allocated</span>
               </div>
            </div>
      ...
      <div className="bg-card border border-border/50 rounded-[3rem] overflow-hidden shadow-xl">
         <div className="p-8 border-b border-border/50 flex items-center justify-between">
            <h3 className="text-2xl font-black tracking-tight flex items-center gap-3">
               ...
               Recent Payout Cycles
            </h3>
            <Button variant="ghost" className="text-xs font-black uppercase tracking-widest text-primary hover:bg-primary/5 transition-all">View Payout History</Button>
         </div>
            ...
            <table className="w-full text-left">
               <thead>
                  <tr className="bg-muted/10 text-[10px] font-black uppercase tracking-widest text-muted-foreground border-b border-border/30">
                     <th className="px-10 py-6">Consignee / Entity</th>
                     <th className="px-10 py-6">Payment Method</th>
                     <th className="px-10 py-6">Treasury ID</th>
                     <th className="px-10 py-6 text-right">Net Amount</th>
                     <th className="px-10 py-6">Approval</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-border/30">
                  {[
                    { entity: 'Indofood Sukses Makmur', type: 'Distributor Account', method: 'BCA Virtual Account', id: 'TX-482910', amount: 'Rp 842,500,000', status: 'PAID' },
                    { entity: 'Budi Santoso (Warung B.)', type: 'Refund/Reversal', method: 'E-Wallet Transfer', id: 'TX-482911', amount: 'Rp 1,200,000', status: 'PENDING' },
                    { entity: 'Wholesale Partner Group', type: 'Commission Split', method: 'Corporate Bank Transfer', id: 'TX-482912', amount: 'Rp 125,000,000', status: 'PAID' },
                  ].map((row, i) => (
                    <tr key={i} className="hover:bg-primary/5 transition-all group">
                       ...
                       <td className="px-10 py-8 text-right font-black italic text-lg">{row.amount}</td>
                       <td className="px-10 py-8">
                          <div className={cn(
...
// Internal icons since Clock/ShieldCheck from lucide might be already imported or not
